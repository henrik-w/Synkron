...
//*** Advanced settings ***
    ...
    el_adv_folder_1.setAttribute("backup_folder_1", backup_folder_1->isChecked() ? QString("1") : QString("0"));
    el_adv_folder_1.setAttribute("update_only_1", update_only_1->isChecked() ? QString("1") : QString("0"));
    ...
    el_adv_folder_2.setAttribute("backup_folder_2", backup_folder_2->isChecked() ? QString("1") : QString("0"));
    el_adv_folder_2.setAttribute("update_only_2", update_only_2->isChecked() ? QString("1") : QString("0"));
...
//*** Advanced ***
    ...
    backup_folder_1->setChecked(el_adv_folder_1.attribute("backup_folder_1").toInt());
    update_only_1->setChecked(el_adv_folder_1.attribute("update_only_1").toInt());
    move->setChecked(el_adv_folder_1.attribute("move").toInt());
    QDomElement el_adv_folder_2 = el_advanced.firstChildElement("folder_2");
    backup_folder_2->setChecked(el_adv_folder_2.attribute("backup_folder_2").toInt());
    update_only_2->setChecked(el_adv_folder_2.attribute("update_only_2").toInt());
...
void SyncPage::saveAs(QString file_name)
{
	...
	QStringList checked;
	for (int n = 0; n < lw_filters->count(); ++n) {
		if (lw_filters->item(n)->checkState()==Qt::Checked) {
			checked << lw_filters->item(n)->text();
		}
	}
...
void SyncPage::load(QDomDocument & domdoc, QString file_name)
{
	...
	for (int n = 0; n < flist.count(); ++n) {
		for (int f = 0; f < lw_filters->count(); ++f) {
			if (lw_filters->item(f)->text() == flist.at(n)) {
				lw_filters->item(f)->setCheckState(Qt::Checked);
			}
		}
	}
